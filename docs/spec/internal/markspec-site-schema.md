...
| **Lock**                | `markspec-lock/v1`                | Frozen sidecar metadata (`.markspec.lock`). ULID-keyed entries with authoring provenance and external sync metadata. See [Traceability](markspec-traceability.md) for lifecycle. | (project file, not in site API)                                                                            |
