 ...
 * `claude` client's `.mcp.json`, which Copilot CLI 1.0.66 also
 ...
 *
 * The local-server schema differs from `claude`: it adds `type`
...
export const copilotDescriptor: McpAdapter = {
  id: "copilot",
  // Both scopes nest under `mcpServers.<name>`, like claude — only the
  // local-server shape and file path differ.
