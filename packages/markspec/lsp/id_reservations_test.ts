...
Deno.test("id_reservations: hasAnyReservations reflects whether any number is reserved", () => {
  teardown();
  assertEquals(hasAnyReservations(), false);
  reserve("STK_", "", 5);
  assertEquals(hasAnyReservations(), true);
  release("STK_", "", 5);
  // release empties (and drops) the bucket, so nothing is reserved.
  assertEquals(hasAnyReservations(), false);
});
